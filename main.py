...
def contact():
    mess._show(title='Contact us', message="Please contact us on : 'rohithsreedharan2004@gmail.com' ")
...
def save_pass():
    ...
    exists1 = os.path.isfile("TrainingImageLabel\passs.txt")
    if exists1:
        tf = open("TrainingImageLabel\passs.txt", "r")
        key = tf.read()
    else:
        master.destroy()
        ...
        if new_pas == None:
            ...
            tf = open("TrainingImageLabel\passs.txt", "w")
            tf.write(new_pas)
    ...
    if (op == key):
        if(newp == nnewp):
            txf = open("TrainingImageLabel\passs.txt", "w")
            txf.write(newp)
        else:
...
window.title("Attendance Management System [Rohithaditya]")
...
trainImg = tk.Button(frame2, text="Save Profile", command=passw ,fg="black"  ,bg="#F5FD03"  ,width=34  ,height=1, activebackground = "white" ,font=('times', 15, ' bold '))
